...
if (length(args) != 1) stop("Usage: Rscript snai1ac_mp_existing_scores_tumor_filter_meta_analysis.R <output_dir>")
...
input <- file.path(tables_dir, "snai1ac_mp_existing_scores_tumor_filter_per_sample.csv")
...
arm_order <- unique(d$arm)
mp_order <- unique(d$MP_id[order(d$MP_id)])
rows <- list()
...
summary$q_value_global <- p.adjust(summary$p_value, method = "BH")
summary$q_value_by_arm <- ave(summary$p_value, summary$arm, FUN = function(x) p.adjust(x, method = "BH"))
summary <- summary[order(summary$arm, summary$MP_id), ]
write.csv(summary, file.path(tables_dir, "snai1ac_mp_existing_scores_tumor_filter_meta_summary.csv"), row.names = FALSE)
...
image(
  ...
  xlab = "",
  ylab = "",
  main = "Pooled SNAI1-ac x MP Spearman r, tumor spots only"
)
